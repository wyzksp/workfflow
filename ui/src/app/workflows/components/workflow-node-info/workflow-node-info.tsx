...
export const WorkflowNodeSummary = (props: Props) => {
    const attributes = [
        ...
        ...(props.node.message
            ...
            : []),
        {title: '开始时间', value: <Timestamp date={props.node.startedAt} />},
        {title: '结束时间', value: <Timestamp date={props.node.finishedAt} />},
        {
            title: '持续时间',
        ...
        {title: '进展', value: props.node.progress || '-'},
        {
            title: '备忘',
            value: (
                <InlineTable
    ...
    if (props.node.type === 'Pod') {
        attributes.splice(2, 0, {title: 'POD名', value: props.node.id}, {title: '主机节点名', value: props.node.hostNodeName});
    }
...
export const WorkflowNodeContainer = (props: {
    ...
    const attributes = [
        ...
        hasEnv(container)
            ...
            : {title: '环境变量', value: <pre className='workflow-node-info__multi-line' />}
    ];
    return (
        <div className='white-box'>
            <div className='white-box__details'>{<AttributeRows attributes={attributes} />}</div>
